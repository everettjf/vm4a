...
public enum OCIMediaType {
    ...
    public static let easyvmConfigV1 = "application/vnd.easyvm.config.v1+json"
    public static let easyvmBundleV1 = "application/vnd.easyvm.bundle.v1.tar+gzip"
}
...
public struct OCIReference: Sendable, Equatable {
    ...
    public static func parse(_ raw: String) throws -> OCIReference {
        ...
        guard let firstSlash = image.firstIndex(of: "/") else {
            throw EasyVMError.message("Registry reference must be fully qualified: <host>/<repo>[:<tag>] (got '\(raw)')")
        }
        let host = String(image[..<firstSlash])
        guard host.contains(".") || host.contains(":") || host == "localhost" else {
            throw EasyVMError.message("Registry reference must start with a host containing '.' (e.g. ghcr.io, docker.io). Got '\(raw)'.")
        }
        let repo = String(image[image.index(after: firstSlash)...])
        guard !repo.isEmpty, !tag.isEmpty else {
            throw EasyVMError.message("Malformed reference '\(raw)'")
        }
...
public struct EasyVMBundleConfig: Codable, Sendable {
    public let schemaVersion: Int
...
public enum OCIAuth: Sendable {
    ...
    public static func fromEnvironment() -> OCIAuth {
        ...
        if let user = env["EASYVM_REGISTRY_USER"], let pw = env["EASYVM_REGISTRY_PASSWORD"], !user.isEmpty {
...
public final class OCIClient: @unchecked Sendable {
    ...
    private func acquireToken(challenge: String) async throws {
        ...
        guard let realm = params["realm"], var comp = URLComponents(string: realm) else {
            throw EasyVMError.message("Malformed registry auth challenge: \(challenge)")
        }
        ...
        guard let http = resp as? HTTPURLResponse, (200..<300).contains(http.statusCode) else {
            ...
            throw EasyVMError.message("Registry auth failed (HTTP \(code)). Set EASYVM_REGISTRY_USER / EASYVM_REGISTRY_PASSWORD if this registry requires credentials.")
        }
        ...
        guard let value = t.token ?? t.access_token else {
            throw EasyVMError.message("Registry returned no token")
        }
        storeToken(value)
    }
    ...
    private func perform(_ builder: () -> URLRequest) async throws -> (Data, HTTPURLResponse) {
        ...
        guard let http = resp as? HTTPURLResponse else {
            throw EasyVMError.message("Non-HTTP response from \(req.url?.absoluteString ?? "?")")
        }
    ...
    public func fetchManifest() async throws -> OCIManifest {
        ...
        guard http.statusCode == 200 else {
            throw EasyVMError.message("Failed to fetch manifest for \(ref.repository):\(ref.tag): HTTP \(http.statusCode) \(String(data: data, encoding: .utf8) ?? "")")
        }
    ...
    public func putManifest(_ manifest: OCIManifest) async throws {
        ...
        guard (200..<300).contains(http.statusCode) else {
            throw EasyVMError.message("Failed to push manifest: HTTP \(http.statusCode) \(String(data: data, encoding: .utf8) ?? "")")
        }
    }
    ...
    public func downloadBlob(descriptor: OCIDescriptor, to fileURL: URL, progress: ((Int64, Int64) -> Void)? = nil) async throws {
        ...
        if http?.statusCode == 401, let challenge = http?.value(forHTTPHeaderField: "Www-Authenticate") {
            ...
            guard let http2 = http, (200..<300).contains(http2.statusCode) else {
                throw EasyVMError.message("Blob fetch failed: HTTP \(http?.statusCode ?? -1)")
            }
    ...
    public func uploadBlob(fileURL: URL, mediaType: String, progress: ((Int64, Int64) -> Void)? = nil) async throws -> OCIDescriptor {
        ...
        guard locResp.statusCode == 202, let location = locResp.value(forHTTPHeaderField: "Location") else {
            throw EasyVMError.message("Blob upload init failed: HTTP \(locResp.statusCode) \(String(data: locData, encoding: .utf8) ?? "")")
        }
        ...
        guard let http = uResp as? HTTPURLResponse else {
            throw EasyVMError.message("Non-HTTP response during blob PUT")
        }
        if http.statusCode == 401, let challenge = http.value(forHTTPHeaderField: "Www-Authenticate") {
            try await acquireToken(challenge: challenge)
            applyAuth(&putReq)
            ...
            guard let http2 = uResp2 as? HTTPURLResponse, (200..<300).contains(http2.statusCode) else {
                throw EasyVMError.message("Blob upload failed after re-auth: HTTP \((uResp2 as? HTTPURLResponse)?.statusCode ?? -1) \(String(data: uData2, encoding: .utf8) ?? "")")
            }
        } else if !(200..<300).contains(http.statusCode) {
            throw EasyVMError.message("Blob upload failed: HTTP \(http.statusCode) \(String(data: uData, encoding: .utf8) ?? "")")
        }
    ...
    public func uploadInlineBlob(data: Data, mediaType: String) async throws -> OCIDescriptor {
        ...
        guard locResp.statusCode == 202, let location = locResp.value(forHTTPHeaderField: "Location") else {
            throw EasyVMError.message("Inline blob init failed: HTTP \(locResp.statusCode) \(String(data: locData, encoding: .utf8) ?? "")")
        }
        let finalURL = try appendQuery(to: absoluteLocation(location), name: "digest", value: "sha256:\(digest)")
        ...
        guard (200..<300).contains(uResp.statusCode) else {
            throw EasyVMError.message("Inline blob upload failed: HTTP \(uResp.statusCode) \(String(data: uData, encoding: .utf8) ?? "")")
        }
    ...
    private func appendQuery(to url: URL, name: String, value: String) throws -> URL {
        guard var comp = URLComponents(url: url, resolvingAgainstBaseURL: true) else {
            throw EasyVMError.message("Invalid URL: \(url)")
        }
        var items = comp.queryItems ?? []
        items.append(.init(name: name, value: value))
        comp.queryItems = items
        guard let out = comp.url else {
            throw EasyVMError.message("Failed to build URL from \(url)")
        }
        return out
    }
}
...
public func packBundleTarball(bundleDir: URL, outputTarGz: URL) throws {
    ...
    task.arguments = ["-czf", outputTarGz.path(percentEncoded: false), "--exclude", ".easyvm-run.pid", "--exclude", ".easyvm-run.log", name]
    let errPipe = Pipe()
    ...
    if task.terminationStatus != 0 {
        ...
        throw EasyVMError.message("tar failed (\(task.terminationStatus)): \(err)")
    }
}
...
public func extractBundleTarball(tarGz: URL, into parentDir: URL) throws {
    ...
    if task.terminationStatus != 0 {
        let err = String(data: errPipe.fileHandleForReading.readDataToEndOfFile(), encoding: .utf8) ?? ""
        throw EasyVMError.message("tar extract failed (\(task.terminationStatus)): \(err)")
    }
}
...
public func ociPush(bundleDir: URL, reference: String, progress: ((String) -> Void)? = nil) async throws {
    ...
    guard FileManager.default.fileExists(atPath: bundleDir.appending(path: "config.json").path(percentEncoded: false)) else {
        throw EasyVMError.message("Not a VM bundle (missing config.json): \(bundleDir.path())")
    }
    let client = OCIClient(ref: ref, auth: .fromEnvironment())

    let tmpDir = FileManager.default.temporaryDirectory.appending(path: "easyvm-push-\(UUID().uuidString)", directoryHint: .isDirectory)
    ...
    let layerDesc = try await client.uploadBlob(fileURL: tarURL, mediaType: OCIMediaType.easyvmBundleV1) { rx, tot in
        if tot > 0 { progress?("  layer: \(rx)/\(tot) bytes") }
    }

    let configBlob = EasyVMBundleConfig(
    ...
    let configDesc = try await client.uploadInlineBlob(data: configData, mediaType: OCIMediaType.easyvmConfigV1)
...
public func ociPull(reference: String, into parentDir: URL, progress: ((String) -> Void)? = nil) async throws -> URL {
    ...
    guard let layer = manifest.layers.first(where: { $0.mediaType == OCIMediaType.easyvmBundleV1 }) ?? manifest.layers.first else {
        throw EasyVMError.message("Manifest has no layers")
    }

    try FileManager.default.createDirectory(at: parentDir, withIntermediateDirectories: true)
    let tmpDir = FileManager.default.temporaryDirectory.appending(path: "easyvm-pull-\(UUID().uuidString)", directoryHint: .isDirectory)
    try FileManager.default.createDirectory(at: tmpDir, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: tmpDir) }
    ...
    throw EasyVMError.message("Pull succeeded but no VM bundle found in \(parentDir.path())")
}
