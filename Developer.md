...
- **GUI Time Machine viewer.** The CLI writes JSONL events; the SwiftUI side that scrubs through them and shows snapshot diffs hasn't been written. See `Sources/VM4ACore/Sessions.swift` for the data shape.
...
- **Resource caps beyond CPU/memory.** Disk quota, IO bandwidth limits, and per-VM CPU shares aren't exposed.
- **`xcode-dev` template's `build.sh`.** Missing because macOS guests can't be installed headlessly today; you have to start with a GUI install, then run `provision.sh` over SSH.
