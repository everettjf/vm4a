...
├── templates/                     ← OCI template recipes (ubuntu-base, python-dev)
...
- **macOS guest support in the CLI is intentionally removed.** Apple's Setup Assistant and the Remote-Login enable step on first boot can't be driven headlessly, so a CLI-spawned macOS guest can never reach an agent-usable state without human input. `VMOSType.macOS` and the VZ-side platform code in `VM4ACore/Core.swift` are kept because the SwiftUI app's "New macOS VM" install flow still uses them; they just aren't exposed via `vm4a create` / `vm4a spawn`.
